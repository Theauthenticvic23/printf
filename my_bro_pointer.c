...
/****************** PRINT POINTER ******************/
/**
 * myini_pointer - Prints the value of a pointer variable
 ...
 */
int myini_pointer(va_list args, char lim[],
	int flags, int width, int precision, int size)
{
	...
	return (write_cynp(lim, ind, length,
		width, flags, padd, extra_c, padd_start));
}

/************************* PRINT NON PRINTABLE *************************/
/**
 * print_CYN - Prints ascii codes in hexa of non printable chars
 * @args: List a of arguments
 * @lim: lim array to handle print
 * @flags:  Calculates active flags
 * @width: get width
	...
	int flags, int width, int precision, int size)
{
	...
	while (str[i] != '\0')
	{
		if (cyn_is_print(str[i]))
			lim[i + offset] = str[i];
		else
			offset += print_extra(str[i], lim, i + offset);
...
int print_revs(va_list args, char lim[],
	int flags, int width, int precision, int size)
{
	char *str;
	int i, count = 0;
 ...
 * Return: Numbers of chars printed
 */
int print_rot13(va_list args, char lim[],
	int flags, int width, int precision, int size)
{
	char x;
	char *str;
